...
Miloco 会用本地账号保护网页面板。首次安装时，打开页面后先创建第一个管理员；
此后每个浏览器都必须先登录，才能查看或操作摄像头、设备、模型设置、Home
Assistant 和日志。

在「用户」页可以新增管理员、重置密码、停用账号或删除账号。Miloco 不允许停用
或删除最后一个已启用的管理员，也不允许在当前浏览器会话中删除自己。

Dashboard 登录是给人使用的。`config.json` 里的本机 `server.token` 是另一套给
`miloco-cli`、OpenClaw、Hermes 和可信自动化使用的机器凭据。不要在浏览器粘贴、
发送或暴露这个 token；浏览器登录后只使用自己的本地会话。
...
- 代码仓库：[github.com/IIIIOvOIIII/lynxloco](https://github.com/IIIIOvOIIII/lynxloco)
- 上游项目：[github.com/XiaoMi/xiaomi-miloco](https://github.com/XiaoMi/xiaomi-miloco)
