...
async def test_omni_config(
    ...
    api_protocol = body.api_protocol if body.api_protocol is not None else omni.api_protocol
    effective_protocol = resolve_api_protocol(api_protocol, model)
    ...
    if not api_key and effective_protocol != "openai_responses":
        return NormalResponse(
            code=0,
            message="ok",
            data={"ok": False, "code": "no_key", "message": "未配置 API Key"},
        )
    result = await _probe.probe_omni(model, base_url, api_key, api_protocol)
    ...
    if result.get("ok"):
        ...
        tested_is_active = (
            ...
            and effective_protocol == resolve_api_protocol(live.api_protocol, live.model)
        )
        if tested_is_active:
