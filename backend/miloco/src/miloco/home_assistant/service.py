...
from miloco.utils.agent_config import mutate_shared_config, update_shared_config
...
class HomeAssistantService:
    ...
    async def update_entity_policies(
        ...
        def apply(existing: dict[str, object]) -> dict[str, object] | None:
            raw_home_assistant = existing.get("home_assistant")
            current_settings = HomeAssistantSettings.model_validate(
                raw_home_assistant if isinstance(raw_home_assistant, dict) else {}
            )
            entities = {
                key: value.model_dump()
                for key, value in current_settings.entities.items()
            }
            ...
            for entity_id in update.entity_ids:
                ...
                current = self._policy_for(current_settings, entity_id)
            ...
            if not updated:
                return None
            return deep_merge(existing, {"home_assistant": {"entities": entities}})

        mutate_shared_config(apply)
