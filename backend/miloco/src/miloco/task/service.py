...
from miloco.rule.schema import SCENE_IID, RuleAction, RuleDirection
...
class TaskService:
    ...
    def set_boundary_actions(self, task_id: str, req: TaskActionsUpdateRequest) -> bool:
        ...
        from miloco.rule.service import _validate_action_slot

        for name, value in slots.items():
            if name.endswith("_actions") and value is not None:
                _validate_action_slot(name, [RuleAction.model_validate(a) for a in value])
