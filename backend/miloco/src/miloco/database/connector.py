...
# v5 = task 运行态重构的 expand-contract 阶段 A (只加列, 阶段 B 才删列)。
...
# Upstream v4 task fields are applied as fork v5 because fork v4 already
# reconciles dashboard authentication and token usage. Keep the upstream field
# names for future merges; all changes are additive.
...
# Detect upstream shapes from high to low. A task-runtime shape deliberately
# reports v4 so fork v5 still reconciles authentication; that step preserves
# existing task actions when all runtime columns are already present.
...
def _create_milestone_rule(cursor: sqlite3.Cursor, task_id: str, now: int) -> None:
    ...
    本函数的执行时刻是未来某天的首次 v4→v5, 抄一份的话服务层改了形状不会有任何
...
def _migrate_v4_to_v5(conn: sqlite3.Connection) -> None:
    """v4 → v5 schema step —— expand-contract 阶段 A: 只加列、只复制, 不删列。

    与 v1→v2 的关键差别: 那一步重建表并 DROP 了列, 不可逆; 这一步全是
    ALTER TABLE ADD COLUMN + UPDATE。旧代码仍可读旧列, 但退代码不会撤销任务
    元数据转换；数据恢复需单独审查, 不能覆盖升级后的新记录。
    ...
    一次事务原子完成:
      ...
      (i) PRAGMA user_version = 5 (同事务)
      COMMIT

    crash 语义同 v1→v2: COMMIT 前 crash → rollback 到 v4 重跑; COMMIT 后 crash →
    user_version=5, 外层步进循环跳过, 不重入。
    ...
    try:
        ...
        _ensure_token_usage_base_url_schema(conn, migration_label="v4→v5")
        # Upstream v4 may already have migrated task actions. Preserve them when
        # importing that history or recovering a database without user_version.
        runtime_present = all(
            name in _table_columns(cursor, table)
            for table, columns in (("task", _V4_TASK_COLUMNS), ("rule", _V4_RULE_COLUMNS))
        ...
        logger.info("v4→v5 added columns: task=%s rule=%s", added_task, added_rule)
        ...
        for row in rules:
            ...
            if stale_debounce:
                logger.info(
                    "v4→v5 reset exit_debounce_seconds %s→%s on rule %s "
        ...
        for task_id, task_rules in by_task.items():
            ...
            if is_conflict:
                ...
                logger.warning(
                    "v4→v5 task %s 名下 %d 条 rule 动作不一致, 不写 task 动作列; "
            ...
            if others - {first["lifecycle"]}:
                logger.warning(
                    "v4→v5 task %s 取首条 rule 的 lifecycle=%s, 忽略 %s",
                    task_id,
                    first["lifecycle"],
                    sorted(others),
                )
            ...
            if target_minutes is None:
                ...
                logger.warning(
                    "v4→v5 task %s 有 on_target_desc 但查不到 duration record 的 "
        ...
        if restored:
            ...
            logger.warning(
                "v4→v5 restored %d rule.enabled=1: %s", len(restored), restored
            )
        ...
        for task_id, directions in rows_by_task.items():
            ...
            if reason:
                ...
                logger.warning(
                    "v4→v5 task %s 迁移后不合法, 置 paused: %s", task_id, reason
                )
        ...
        for row in terminate_when_rules:
            logger.warning(
                "v4→v5 rule %s (task %s) 的 terminate_when 不再生效: %s",
                row["id"],
                row["task_id"],
                row["terminate_when"],
            )
