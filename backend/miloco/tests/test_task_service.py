...
@pytest.mark.parametrize("slot", ["on_enter_actions", "on_exit_actions", "on_target_actions"])
@pytest.mark.parametrize("iid", ["activate", "trigger", "scene"])
def test_task_actions_preserve_ha_trigger_cooldown_policy(service, slot, iid):
    from miloco.middleware.exceptions import ValidationException
    from miloco.task.schema import TaskActionsUpdateRequest

    _mk_task(service)
    before = service.repo.get_full_view("t1")
    action = {"did": "ha:primary:scene.test", "iid": iid}
    with pytest.raises(ValidationException, match="idempotent|cooldown"):
        service.set_boundary_actions("t1", TaskActionsUpdateRequest(**{slot: [action]}))
    assert service.repo.get_full_view("t1") == before


def test_task_actions_keep_valid_ha_action_source(service):
    from miloco.task.schema import TaskActionsUpdateRequest

    _mk_task(service)
    action = {"source": "home_assistant", "did": "ha:primary:light.test", "iid": "on", "value": True}
    assert service.set_boundary_actions("t1", TaskActionsUpdateRequest(on_enter_actions=[action]))
    assert service.get_full_view("t1").actions.on_enter_actions == [action]
