...
def _mock_omni_config(monkeypatch):
    ...
    monkeypatch.setattr("miloco.config.get_settings", lambda: _FakeSettings())
...
async def test_tick_drive_probe_success_recovers_to_closed(
    ...
    async def _fake_probe(model, base_url, api_key, api_protocol):
        assert api_protocol == "openai_chat_completions"
...
async def test_responses_without_key_still_runs_tick_visual_probe(monkeypatch):
    """Responses 免 Key 是 adapter 契约，tick 不得在调用前误报 no_key。"""
    from miloco.perception import processor as _processor

    class _ResponsesOmni:
        model = "local-vision"
...
async def test_probe_exception_falls_back_to_failure_record(
    monkeypatch, _mock_omni_config
):
    ...
    monkeypatch.setattr("miloco.perception.engine.omni.probe.probe_omni", _boom)
...
async def test_probe_cancelled_falls_back_to_open_recoverable(
    monkeypatch, _mock_omni_config
):
