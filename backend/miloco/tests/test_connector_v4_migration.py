...
@pytest.mark.parametrize("stored_version", [0, 4])
def test_fork_v4_task_actions_are_migrated_and_auth_survives(v2_db, stored_version):
    """Fork v4 predates task columns; its version must not skip this migration."""
    import miloco.database.connector as connector_module

    action = [{"source": "home_assistant", "did": "ha:primary:light.test", "iid": "on"}]
    _seed(v2_db, lambda c: (
        _add_task(c, "fork-task"),
        _add_rule(c, "fork-rule", "fork-task", actions=json.dumps(action)),
    ))
    # Run only the already-released fork chain to produce its actual v4 shape.
    db = connector_module.SQLiteConnector()
    with db.get_connection() as conn:
        db._create_token_usage_table(conn)
        db._create_token_usage_daily_table(conn)
        conn.commit()
        connector_module._migrate_v2_to_v3(conn)
        connector_module._migrate_v3_to_v4(conn)
        auth_shape = conn.execute("SELECT sql FROM sqlite_master WHERE name='dashboard_user'").fetchone()[0]
        conn.execute(f"PRAGMA user_version = {stored_version}")
        conn.commit()
    _migrate(v2_db)
    conn = _raw(v2_db)
    assert "direction" in {r[1] for r in conn.execute("PRAGMA table_info(rule)")}
    assert json.loads(conn.execute("SELECT on_enter_actions FROM task WHERE task_id='fork-task'").fetchone()[0]) == action
    assert conn.execute("PRAGMA user_version").fetchone()[0] == 5
    assert conn.execute("SELECT sql FROM sqlite_master WHERE name='dashboard_user'").fetchone()[0] == auth_shape
    assert conn.execute("PRAGMA foreign_key_check").fetchall() == []
    conn.close()


@pytest.mark.parametrize("stored_version", [0, 4, 5])
def test_upstream_task_actions_are_not_recopied_on_fork_reconciliation(v2_db, stored_version):
    _seed(v2_db, lambda c: (_add_task(c, "t1"), _add_rule(c, "r1", "t1")))
    _migrate(v2_db)
    conn = _raw(v2_db)
    conn.execute("UPDATE task SET on_enter_desc = 'updated task action'")
    conn.execute("UPDATE rule SET action_descriptions = ?", (json.dumps(["stale rule action"]),))
    conn.execute(f"PRAGMA user_version = {stored_version}")
    conn.commit()
    conn.close()
    _migrate(v2_db)
    conn = _raw(v2_db)
    assert conn.execute("SELECT on_enter_desc FROM task").fetchone()[0] == "updated task action"
    assert conn.execute("SELECT COUNT(*) FROM rule").fetchone()[0] == 1
    assert conn.execute("PRAGMA user_version").fetchone()[0] == 5
    conn.close()
