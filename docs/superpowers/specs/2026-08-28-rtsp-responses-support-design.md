...
原 mandatory 本地 CI 的 PyAV native abort/segfault 已定位为同一 `InputContainer` 在 demux/decode 活跃时被其他线程并发关闭。生命周期 round 1 以确定性 RED 证明原实现存在 overlap，随后改为 stop/cancel 只发信号并等待 worker、由 demux/decode owner 在退出路径同线程关闭；配置给 PyAV 的 open/read timeout 均为 8 秒，停止等待因此以当前 I/O timeout 剩余时间为上界。修复后 RTSP session 32 passed，camera + session + H.264/H.265 perception + 真实 Uvicorn WebSocket E2E 198 passed，scoped Ruff/format/ty 通过。按恢复边界仅运行一次完整 backend 序列，结果为 3641 passed、4 个既有 macOS node-monitor 资源采样断言失败、exit 1；没有 exit 139、abort/segfault、新增 macOS Python crash report 或 core 文件。未修改 PyAV/OpenCV 版本和 `local-ci.sh`。后续 `./scripts/local-ci.sh --tests` 与完整 `./scripts/local-ci.sh` 均通过脚本定义的 6/6 门禁；脚本仍按既有规则排除 3 个 macOS node-monitor/smaps 用例，范围外 node-monitor/type/format 基线没有被误报为已清零。

round 2 进一步把 owned task 的 reap/join 设为不可被重复 cancel 中断：清理循环显式取回 worker/opener 的结果或异常，旧 decode/container 或 open-in-progress 完成并清除引用后才重新传播原始 `CancelledError`，因此并发 `stop()`/`start()` 不会与旧输入 worker 重叠。直播 smoke 不再调用可被环境变量覆盖的通用 settings loader；它只从 `MILOCO_HOME/config.json` 的 no-follow 文件描述符读取持久化 `server.token`，要求 canonical regular non-symlink、文件 owner 为当前 euid 且 group/other bits 为零，父目录同 owner 且不可 group/other write，0755 父目录仍允许。JSON/schema/token 缺失、不安全权限、symlink 和 env-only 均以统一安全消息 exit 2；file 与冲突 env 同时存在时 file 优先。首帧后 deadline 前 WebSocket 断线 exit 4，SIGTERM exit 143，两者均清理连接且不打印成功指标；正常输出的 sample seconds 与 fps 使用真实有效 elapsed。round 2 focused 矩阵为 210 passed、1 skipped，skip 是非 root 环境无法安全构造 wrong-owner 文件；完整 backend 不按本轮约束重复执行。

round 3 将“不可中断等待”和“清理后传播 outcome”拆成集中但独立的 helper。传播优先级固定为 cancellation-first：没有 owned failure 时重新抛原 `CancelledError`；单个 decode/opener/close failure 直接作为 cancellation cause；多个失败以 `BaseExceptionGroup` 作为 cause。没有 cancellation 时，单 failure 或 failure group 必须正常抛给既有安全错误分类或 task result，不得返回成功。close failure 不再被静默吞掉，decode 与 close 同时失败时两者都保留；实现不新增 raw error、URI 或凭据日志。session 42 passed，double-cancel 两族 25 轮共 175 cases 稳定通过，focused 矩阵为 217 passed、1 skipped，scoped Ruff/format/ty 与 diff/leak 门禁通过。
