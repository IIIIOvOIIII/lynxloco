...
- Current work: Deployed the dashboard-auth release to production `miloco.esxi`.
- Expected result: Production runs the dashboard-auth build, existing Miloco configuration is preserved, and first-admin setup is available without exposing the dashboard unauthenticated.
- Result: Achieved. CO `CHG260831002` was approved by AI, reached `Implement`, and closed `Successfully Closed`. Exact deployed source SHA is `9f1d344426f47427d51edfd5b26acb6d1d496f2a`; release version is `2026.8.6.post1.dev259+g9f1d34442`; OpenClaw plugin version is `2026.8.6-post1.dev259`. The deployment used the official installer/sync package path on the existing OpenClaw Miloco home, not the Docker release-controller path. A pre-change backup was created at `/root/miloco-backup-CHG260831002-9f1d344-20260831081434`. Post-change checks passed: Miloco service running/managed, local and LAN `/health` OK, dashboard root HTTP 200, `/api/auth/status` returned `needs_setup=true`, service-token API remained functional, and sanitized config summary still showed server, agent, camera, Omni/OpenAI, Home Assistant, and service token presence with 3 RTSP sources and 4 cameras visible by API. No secrets, RTSP URLs, API keys, raw camera frames, or raw model responses were printed.
- Next step: Lynx should complete the first-admin setup wizard in the browser with the desired username/password. After that, a follow-up verification can confirm the dashboard-auth user count increased while the same sanitized Xiaomi/RTSP/model/Home Assistant configuration remains present.
