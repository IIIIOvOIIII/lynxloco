...
## 2026-08-28 23:53 SGT

- Current work: 完成同一不可变 release 的 ai-lab01/ai-lab02 双机实验室验收；lab02 在用户扩容内存后复用既有 receipt-bound archive，未重建、未二次 deploy。
- Expected result: 两台主机均运行 runtime SHA `644406e36c621dfad55939686d315a2d3ddc955c`，绑定 archive SHA-256 `8a057845415fa2a6d820449a9dc4744d4a18fb717300930174f3dd05e0522663`，健康、资源限制、owner-only state、HTTP/UI 与 acceptance proof 均满足，真实端点保持显式测量边界。
- Result: Achieved。lab01 final verify/status 都通过，状态为同 SHA、`miloco-lab:644406e36c621dfad55939686d315a2d3ddc955c`、`healthy`；CPU `3`、memory `3072m`、UID/GID `10001:10001`、state `10001:10001:700`、restart `0`、current-process OOM `false`、free disk `9025812` KiB。lab01 的已接受 UI 验证保持有效。lab02 preflight 通过（Docker `26.1.5+dfsg1`、Compose `2.26.1-4`、port `1810`、free disk `25352376` KiB），final verify/status 同样通过；当前容器同 SHA、`healthy`、CPU `1.25`、memory `1536m`、UID/GID `10001:10001`、state `10001:10001:700`、restart `0`、current-process OOM `false`、final free disk `24126448` KiB。lab02 `GET /` 为 HTTP `200`，required receipt-bound fixture acceptance 已由 deployed-release verification 确认；浏览器加载 dashboard、显示 `g644406e36` build label 和导航，无 console error。五分钟、十个有界 status sample 始终维持 exact SHA/healthy/restart `0`/OOM `false`。首个 `not deployed` 读取发生在远端 transaction 尚未完成时；该 transaction 后续正常退出，controller 已确认 healthy container/current record，继续验收时未运行第二次 deploy。`MILOCO_RTSP_TEST_URL`、`MILOCO_RESPONSES_BASE_URL`、`MILOCO_RESPONSES_MODEL` 均未设置，因此 `real_camera=not_measured`、`real_vlm=not_measured`。ai-lab02 已扩容；先前经批准临时停止的 `openobserve` 和 `openobserve-fluent-bit` 仍保持 stopped，本任务未操作它们或 `lab-mariadb`。无应用日志、secret-bearing runtime configuration 或生产系统访问；没有 upstream push。
- Next step: 该 documentation-only acceptance commit 晚于 deployed runtime SHA；由主控制器继续 Task 5 的独立回滚/完整回归/交接流程，除非另行授权，不重建该 release、不重启或删除无关 lab 服务。

## 2026-08-29 00:31 SGT

- Current work: 完成 Task 5 的 lab01 回滚演练、Task 5A smoke 兼容修复后的最终仓库门禁、全分支与不可变产物安全扫描、双机容器结构检查及最终文档收口。
- Expected result: 不制造历史版本、不重建或部署后续 HEAD；lab01 必须最终回到 deployed runtime SHA `644406e36c621dfad55939686d315a2d3ddc955c` 并保持健康；所有新回归必须清零，仓库既有 baseline 单独记录；文档必须区分 runtime、后续代码/文档 HEAD 与真实端点 `not_measured` 边界。
- Result: Achieved with explicit repository baselines。回滚前 lab01 `current` 为 `644406e36c621dfad55939686d315a2d3ddc955c`、`previous` 不存在；另一 release `0d0acd53ca04bc0696eb1dd66d85b6229f451813` 未具备 acceptance proof，不能冒充 capable previous。因此按批准规则对当前 capable SHA 做同 SHA re-activation，`version_differentiation=not_applicable`；最终 `verify`/`status` 均为 exact SHA、image `miloco-lab:644406e36c621dfad55939686d315a2d3ddc955c`、`healthy`，release checksum、artifact record、acceptance marker、archive digest `8a057845415fa2a6d820449a9dc4744d4a18fb717300930174f3dd05e0522663` 和 mode-`0444` receipt 保持完整。首次 `local-ci --tests` 暴露 uv 风格 symlink venv 的真实 smoke 兼容缺口；Task 5A `8480c6c5956236b71fffc88df4f5dfe7cc6443b0` 修复并经独立复审 CLEAN，未部署。修复后 `local-ci --tests` 6/6 通过，仅保留脚本既有 3 项 Darwin node-monitor/smaps 排除；deployment contract 181 passed；CLI 646 passed；Web 383 passed/1 skipped、typecheck/build 通过；全库 Ruff lint 与 `git diff --check` 通过。只读全库 format 仍有 298 个既有文件、`ty` 仍有 1007 条既有 diagnostics，未运行会改写全库的 `task lint`。从 feature base `e900529fe0203a4b6a0a5487987a633887cf7b7f` 到 local code HEAD `8480c6c5956236b71fffc88df4f5dfe7cc6443b0` 的新增行扫描，以及 runtime archive 33 个顶层成员/329 个递归文件扫描，均未发现 private-key marker、cloud-token form、credential-bearing RTSP URI、长 base64 图片、raw runtime payload artifact、禁止路径、危险成员类型或 credential-like filename；Authorization/Bearer 字面量仅存在于合成测试。双机容器只检查结构和 env 名称，均无 API-key 或 RTSP-credential env 名称，未输出值。fixture acceptance 在两机均覆盖 RTSP H.264/H.265 perception、共享 session live，以及 Responses JSON/SSE/no-key/合成 Bearer；`real_camera=not_measured`、`real_vlm=not_measured`。deployed runtime 仍为 `644406e...`；后续 docs evidence `3982f7a...`、Task 5A `8480c6c...` 与本次 closeout docs HEAD 均未 build/deploy。lab02 扩容后资源与五分钟健康证据保持有效；`openobserve`/`openobserve-fluent-bit` 继续保持经批准的 temporary stopped 状态，`lab-mariadb` 保持 running/healthy，本任务未操作这些服务。不可变发布继续累积，不自动删除历史；低于 5 GiB 时只 fail closed 停止。没有 upstream push。
- Next step: 提交本次四份授权文档；由主控制器执行 Task 5 独立复审及最终 whole-branch review。任何后续 runtime 变更必须从新的 exact clean SHA 重新 build/deploy；真实摄像机与真实本地 VLM 验收仍需另行提供端点和授权。

## 2026-08-30 02:28 SGT

- Current work: 用户已批准 RTSP OOM 根因修复书面规格；使用 writing-plans 将 RTSP-only 感知入口采样、每源帧数/128 MiB 字节上限、转码 admission-before-copy、每 feed 一 active 加一 latest pending，以及 build/lab/生产闭环拆成可逐项执行的计划。
- Expected result: 计划必须逐条覆盖批准规格，提供精确文件、接口、RED→GREEN 测试、只读门禁、独立审查、逐任务提交、单次 immutable build、lab01→lab02 顺序验收、新 CO/PAM 生产部署、十分钟 `<2.5 GiB`/零重启/零 OOM 门禁和 offline rollback；本阶段不修改运行时代码或访问主机。
- Result: Achieved pending user plan review。计划保存于 `docs/superpowers/plans/2026-08-30-rtsp-memory-bounds.md`，共 9 个任务、1,091 行。自检修正了 retention 丢帧计数不得写入既有 dropped-windows 语义、异常 payload 诊断必须有界、shell 命令工作目录隔离和实施基线描述；占位符、规格覆盖、接口/生命周期、单次构建、旧 release 保留、生产 offline 边界与 `git diff --check` 均通过。未修改运行时代码，未访问 ai-lab01/02 或 docker.esxi，生产 Miloco 继续保持停止。
- Next step: 提交书面规格批准状态、实施计划与本进度记录；用户批准计划后，按既定 superpowers:subagent-driven-development 从 Task 1 的 RED 缓冲区 retention 测试开始，Task 1 独立规格与质量复审无 Critical/Important 前不进入 RTSP adapter 接线。

## 2026-08-30 03:33 HKT

- Current work: 完成 RTSP memory-bounds Task 5 的全分支本地 gate、首次 whole-branch review、单项 Important finding 修正及 scoped correction re-review，并据此冻结候选文档；文档阶段仅修改本进度文档和批准规格，未 build candidate archive、访问主机或部署。
- Expected result: RTSP 感知必须按 host-monotonic clock 限制为配置的 perception FPS；每个 RTSP 窗口最多保留 `ceil(target_fps * window_size_seconds) + 1` 帧，每源 active+drained 视频 payload 不超过 `128 MiB`，两源合计上限约 `256 MiB`；转码路径必须 admission-before-copy、timestamp history 最多 256 项、每 feed 只保留一个 active push 和一个 latest pending frame。完整 focused/backend/Web/deploy/shell/static/scope/privacy gate 不得出现新回归，Task 1–4 独立复审必须 CLEAN，真实端点与生产证据不得由 fixture 冒充。
- Result: Achieved for implementation and local acceptance。Task 1–4 的独立规格/质量复审均 CLEAN。首次 whole-branch review 发现唯一 Important finding：每帧先创建 event-loop callback handle，导致 yield 前 98 个 intermediate ndarray 均被保留；修正提交 `7ee992645e0ac5ca0ab5c27bd25c3233d291396f` 的 behavioral RED 为 `98 retained`，GREEN 为 `0 intermediate retained`，affected suite 为 `56 passed, 1 skipped, 7 warnings`，修正后 final focused RTSP repair matrix 为 `172 passed, 1 skipped, 7 warnings`。七条 warning 均与 Task 4 相同：`websockets.legacy` deprecated 1 条、`WebSocketServerProtocol` deprecated 1 条、legacy `ws_handler` 二参数 deprecated 5 条。scoped correction re-review 判定原 finding `ADDRESSED`、规格 `COMPLIANT`、质量 `APPROVED`，Critical/Important/Minor 均为 none，允许 freeze。最终本地 bound 为：callback ingress 对每个 feed 使用独立同步的 latest slot，hub 最多持有 active ndarray 加 latest pending ndarray，并且最多调度一个不携带 frame reference 的 handoff；hub ingress 不复制 ndarray。Task 5 原文的 inherited backend command 因六个测试路径在当前 HEAD 与只读基线 `37e1d58c82814448c7a3381c256e3f4aca6b1b71` 均不存在而 `exit 4 / no tests ran`，因此仅记为既有计划路径缺陷、绝不计为新 green proof；映射到现有 camera/admin/Omni 路径的补充矩阵为 `262 passed`。focused Web command 为 `3 files / 24 passed`；其中列出的两个 Omni 文件在 HEAD/基线均不存在，Vitest 未收集它们，故同样不把不存在的文件计作 green，完整 Web suite 另行覆盖现有测试。`./scripts/local-ci.sh --tests` 为 `6/6` 通过（Hermes `185 passed, 2 skipped`，backend 成功）；同一 backend 命令独立保留精确计数为 `3932 passed, 1 skipped, 90 warnings`。完整 Web 为 `23 files / 403 passed, 1 skipped`，typecheck/build 通过；Web 测试有 15 条 Node localStorage ExperimentalWarning，Vite 仅保留既有 `508.43 kB > 500 kB` chunk 提示。计划中的单文件 node-monitor 路径同样在 HEAD/只读基线不存在并 `exit 4`，不计 green；现有完整 node-monitor 目录补充验证为 `127 passed, 1 StarletteDeprecationWarning`。local-ci 的既有 Darwin 规则仍会容忍最多 3 项 node-monitor/smaps 平台失败，但本次 backend 返回成功，未实际触发该排除。deployment contract 为 `196 passed`；六个 shell 文件 `bash -n` 通过；scoped Ruff check 通过，Ruff format 为 `4 files already formatted`；`git diff --check`、clean pre-doc status、scope audit 均通过。scope 从基线到 HEAD 限于四个规格 runtime 模块、focused tests、批准 docs，以及 SDD ledger 已裁决的一项精确例外：`rtsp_camera_source.py` 仅新增一行 replacement DID reconciliation；无其他扩张。changed-text privacy scan 为 URI userinfo `0`、Authorization value `0`、base64 image material `0`、API-key literal `0`、provider body `0`。完整 backend 的 90 条 inherited/non-blocking warnings 分类为 Starlette 1、三个 legacy edge helper 各 1、tar extraction 1、websockets/Uvicorn `1 + 1 + 9`、错误 asyncio mark 7、AsyncMock 未 await runtime warning `65 + 2`。原有全局 connect/read `15s` 与 Responses visual POST `30s` 路径未修改。真实 RTSP camera、真实 Responses provider、candidate archive、`ai-lab01.esxi`、`ai-lab02.esxi`，以及生产十分钟 memory `<2.5 GiB`、restart 与 OOM 证据均为 `not_measured`；本地测试不是这些证据。
- Next step: 从 clean candidate exact SHA 只 build 一次，独立复核 immutable receipt/archive，然后部署同一 bytes，严格按 `ai-lab01.esxi` → `ai-lab02.esxi` 顺序验收。生产继续停止；只有双 lab 通过后，才可在新 CO/PAM 下执行生产部署与真实 RTSP/Responses/十分钟 memory-restart-OOM 验收。
